...
// Constructor
NewMotor::NewMotor(int id, Adafruit_MotorShield *pShield, uint8_t phaseA, uint8_t phaseB, unsigned int encoderCPR,double Kp,double Ki,double Kd,int dir)
:_encoder(phaseA,phaseB),
_pShield(pShield),
_pid(&_input,&_output,&_setpoint,Kp,Ki,Kd,dir)
{
    // Motor stuff
    _id = id;
...
void NewMotor::setPIDMode(int mode)
{
	_pid.SetMode(mode);
}

int NewMotor::getPIDMode()
{
	return _pid.GetMode();
}
