classdef DCMotor < arduinoio.LibraryBase
    
    properties(Access = private, Constant = true)
    GET_RAD = hex2dec('00')
    GET_RAD_SEC = hex2dec('01')
    GET_COUNTS = hex2dec('02')
    MOTOR_CREATE = hex2dec('03')
    GET_COUNTSSEC = hex2dec('04')
    SET_RADSEC = hex2dec('05')
    UPDATE_MOTORS = hex2dec('06');
    UPDATE_VOLTAGES = hex2dec('07');
    end
    
    properties(Access = protected, Constant = true)
        LibraryName = 'MotorLibrary/DCMotor'
        DependentLibraries = {}
        ArduinoLibraryHeaderFiles = {'NewMotorLibrary/NewMotorLibrary.h'}
        CppHeaderFile = fullfile(arduinoio.FilePath(mfilename('fullpath')), 'src', 'MatlabMotor.h')
        CppClassName = 'MatlabMotor'
    end
    
    properties(Access = private)
        ResourceOwner = 'MotorLibrary/DCMotor';
    end
    ...
    methods(Access = public)
        function obj = DCMotor(parentObj,inputPins)
            obj.Parent = parentObj;
            obj.Pins = inputPins;
            count = getResourceCount(obj.Parent,obj.ResourceOwner);
            if count > 3
                error('You can only have 3 Motors');
            end
...
%         end
    ...
    methods(Access = public)
         ...
         function [] = updateVoltages(obj,voltageArray)
             bat = 11.4;
             % Convert to driver format
             for i = 1:3
                 voltageArray(i) = 255*voltageArray(i)/bat;
             end
             
             cmdID = obj.UPDATE_VOLTAGES;
             input1 = [typecast(int16(voltageArray(1)),'uint8')];
             input2 = [typecast(int16(voltageArray(2)),'uint8')];
             input3 = [typecast(int16(voltageArray(3)),'uint8')];
